    ...
    ////////////////////////////////////////////
    /// De modificat in timpul laboratorului ///
    ////////////////////////////////////////////

    /// Incrucisare
    /// Cu doua puncte de rupere: Pentru fiecare pereche de cromozomi selectati se genereaza cu probabilitate uniform distribuita doua puncte de rupere si se interschimba partea de mijloc
    /// Cu masa de biti: Se genereaza pentru fiecare pereche un sir de 1 si 0 de lungime egala cu cea a cromozomilor pe fiecare pozitiedin masca, primul fiu este obtinut astfel: daca avem 0 se preai a informatia din parinte 1, astfel din parinte 2; pentru al doilea fiu se lucreaza analog; exemplu: cromozom1 10010, cromozom2 11001 si masca 11011; fiu a) 11001; fiu b) 10010

    /// Selectie
    /// Selectie turneu: am in urna n bile fiecare asociata unui individ. Extrag in acelasi timp doua bile. Selectez pentru mai departe o copie a individului cu fitness cel mai mare dintre cei doi inscrisi pe bile. Se reintroduc cele doua bile si se repeta pana la consumarea numarului de locuri pentru selectie

    /// Mutatie
    /// Mutatie pe gena: Nu mai schimbam bitul din 1 in 0 si vice versa ci ne uitam la valoarea sa si la valoarea vecinilor, le adunam modulo 2 iar rezultatul va fi noua valoare. Exemplu: 11001 -> 10001. Toate mutatiile au loc in paralel (in acelasi timp), nu secvential (una dupa alta)
    ...
    int main() {
        ...
        /// Incrucisare cu doua puncte de rupere: Pentru fiecare pereche de cromozomi selectati se genereaza cu probabilitate uniform distribuita doua puncte de rupere si se interschimba partea de mijloc

        fout << "\n";
        fout << "#######################\n";
        fout << "# De modificat in cod #\n";
        fout << "#######################\n\n";

        vector<Individ> individ_dupa_incrucisare_cu_doua_puncte_de_rupere;
        individ_dupa_incrucisare_cu_doua_puncte_de_rupere.resize(numar_cromozomi);

        for (int i = 0; i < numar_cromozomi / 2; i++) {
            /// Copiaza reprezentarile binare in noul vector
            for (int j = 0; j < numar_biti; j++) {
                individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i].reprezentare_binara[j] = individ[2 * i].reprezentare_binara[j];
                individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i + 1].reprezentare_binara[j] = individ[2 * i + 1].reprezentare_binara[j];
            }

            /// Afiseaza perechea initiala
            fout << "-> Perechea initiala:\n";
            for (auto bit : individ[2 * i].reprezentare_binara) {
                fout << bit;
            }
            fout << "\n";
            for (auto bit : individ[2 * i + 1].reprezentare_binara) {
                fout << bit;
            }

            /// Genereaza doua puncte de rupere
            int punct_stang, punct_drept;
            punct_stang = rand() % 20; /// chiar daca sunt 22 de biti, ar arata mai bine incrucisarea asa
            punct_drept = rand() % 20;

            /// Asigura-te ca punct_stang < punct_drept pentru a fi un interval valid
            if (punct_stang > punct_drept) {
                int aux = punct_stang;
                punct_stang = punct_drept;
                punct_drept = aux;
            }

            /// Afiseaza cele doua puncte de rupere
            fout << "\n\n-> Cele doua puncte de rupere: " << punct_stang << " si " << punct_drept << "\n\n";

            /// Copiaza bitii pana la punct stanga
            for (int punct = 0; punct < punct_stang; punct++) {
                individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i].reprezentare_binara[punct] = individ[2 * i].reprezentare_binara[punct];
            
                individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i + 1].reprezentare_binara[punct] = individ[2 * i + 1].reprezentare_binara[punct];
            }

            /// Interschimba bitii pana la punct drept
            for (int punct = punct_stang; punct < punct_drept; punct++) {
                individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i].reprezentare_binara[punct] = individ[2 * i + 1].reprezentare_binara[punct];
            
                individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i + 1].reprezentare_binara[punct] = individ[2 * i].reprezentare_binara[punct];
            }

            /// Copiaza bitii pana la numar de biti
            for (int punct = punct_drept; punct < numar_biti; punct++) {
                individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i].reprezentare_binara[punct] = individ[2 * i].reprezentare_binara[punct];
            
                individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i + 1].reprezentare_binara[punct] = individ[2 * i + 1].reprezentare_binara[punct];
            }

            /// Afiseaza perechea incrucisata
            fout << "->Perechea incrucisata:\n";
            for (int punct = 0; punct < punct_stang; punct++) {
                fout << individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i].reprezentare_binara[punct];
            }
            fout << " ";
            for (int punct = punct_stang; punct < punct_drept; punct++) {
                fout << individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i].reprezentare_binara[punct];
            ...
            fout << "\n";
            for (int punct = 0; punct < punct_stang; punct++) {
                fout << individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i + 1].reprezentare_binara[punct];
            }
            fout << " ";
            for (int punct = punct_stang; punct < punct_drept; punct++) {
                fout << individ_dupa_incrucisare_cu_doua_puncte_de_rupere[2 * i + 1].reprezentare_binara[punct];
